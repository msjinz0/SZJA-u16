  ...
  return<div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{width:400,maxWidth:"95vw"}}>
    ...
    <div style={{background:C.card,border:"1px solid "+C.brd,borderRadius:16,padding:24}}>
      {mode==="coach"?<div>
        ...
        <div style={{marginBottom:14}}><label style={{fontSize:11,color:C.txM,display:"block",marginBottom:5,fontWeight:600}}>Válaszd ki a neved</label><select value={selPlayer} onChange={e=>{setSelPlayer(e.target.value);setPin("");setErr("")}} style={{width:"100%",padding:"11px 14px",borderRadius:10,background:C.bg2,border:"1px solid "+C.brd,color:C.tx,fontSize:13,outline:"none",boxSizing:"border-box"}}><option value="">-- Válassz játékost --</option>{playerList.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div>
        {selPlayer&&<div>
          <p style={{fontSize:12,color:C.txM,marginBottom:14}}>Írd be a 4 jegyű PIN kódod</p>
          <div style={{display:"flex",justifyContent:"center",gap:10,marginBottom:20}}>{[0,1,2,3].map(i=><div key={i} style={{width:44,height:54,borderRadius:12,background:C.bg2,border:"2px solid "+(pin.length>i?C.a:C.brd),display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,fontWeight:800,color:C.tx}}>{pin[i]?"●":""}</div>)}</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,maxWidth:240,margin:"0 auto"}}>{[1,2,3,4,5,6,7,8,9,null,0,"⌫"].map((n,i)=><button key={i} onClick={()=>{if(n==="⌫"){setPin(p=>p.slice(0,-1));setErr("")}else if(n!==null&&pin.length<4){const np=pin+String(n);setPin(np);setErr("");if(np.length===4){setLoading(true);supabase.from("players").select("*").eq("id",Number(selPlayer)).eq("pin",np).eq("active",true).single().then(({data})=>{setLoading(false);if(data)onLogin({type:"player",user:data});else{setErr("Hibás PIN");setPin("")}})}}}} style={{height:48,borderRadius:10,border:"1px solid "+C.brd,background:n===null?"transparent":C.bg2,color:C.tx,fontSize:18,fontWeight:700,cursor:n===null?"default":"pointer",visibility:n===null?"hidden":"visible"}}>{n}</button>)}</div>
          <div style={{marginTop:14,padding:8,background:C.bg2,borderRadius:8,fontSize:10,color:C.txD,textAlign:"center"}}>Kérd el a PIN kódodat az edzőtől!</div>
        </div>}
        {!selPlayer&&<div style={{textAlign:"center",padding:20,color:C.txD,fontSize:12}}>Válaszd ki a neved a fenti listából</div>}
      </div>}
